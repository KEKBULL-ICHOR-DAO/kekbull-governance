...
pub fn create_spl_token_account_signed<'a>(
    ...
    let space = if spl_token_program_id.eq(&spl_token_2022::id()) {
...
pub fn assert_is_valid_spl_token_account(account_info: &AccountInfo) -> Result<(), ProgramError> {
    ...
    if account_info.owner != &spl_token_2022::id() && account_info.owner != &inline_spl_token::ID {
...
/// is initialized and belongs to spl_token or Token-2022.
///
/// **KEKBULL fork (vs stock GovER5 3.1.2):** stock `valid_mint_length` requires
/// `mint_data[Mint::LEN] == AccountType::Mint`. Modern Token-2022 mints with
/// extensions (e.g. TransferFeeConfig) place AccountType at
/// `BASE_ACCOUNT_LENGTH` (165) after zero padding, so `data[82] == 0` and
/// CastVote/FinalizeVote fail with SplTokenInvalidMintAccountData (548).
/// Prefer `PodStateWithExtensions::<PodMint>::unpack` - same family as
/// `get_current_mint_fee` on the deposit path - then fall back to the stock
/// length/initialized checks for legacy layouts.
pub fn assert_is_valid_spl_token_mint(mint_info: &AccountInfo) -> Result<(), ProgramError> {
    ...
    // inline_spl_token is used to avoid including the whole package.
    if mint_info.owner != &spl_token_2022::id() && mint_info.owner != &inline_spl_token::ID {
        return Err(GovernanceError::SplTokenMintWithInvalidOwner.into());
    }

    let data = mint_info.try_borrow_data()?;

    // Extended Token-2022 (TransferFeeConfig, etc.) and valid base mints.
    if PodStateWithExtensions::<PodMint>::unpack(&data).is_ok() {
        return Ok(());
    }

    // Stock GovER5 path for exact Mint::LEN / legacy layouts.
    if !valid_mint_length(&data) {
